...
class DBHlpr(context: Context) : SQLiteOpenHelper(context, "details.db", null, 1){
    ...
    fun retrive(s1: String): String {
      //  sqLiteDatabase=writableDatabase
        ...
        var c : Cursor = sqLiteDatabase.query("students",null,"Name=?", arrayOf(s1),null
        ,null,null)
    ...
    fun updateloc(s1: String, s2: String): Int {
        ...
        val cv = ContentValues()
        cv.put("Location",s2)
       var j = sqLiteDatabase.update("students",cv,"Name = $s1", null)
        return j
    }

    fun delname(s1: String) {
        sqLiteDatabase.delete("students","Name = $s1",null)
    }
}